...
data = amcat.features(conn, articleset_id=4073, unit_level='article') # per artikel
data = amcat.features(conn, articleset_id=4073, unit_level='paragraph') # per paragraaf (per 'sentence' ook mogelijk)

## Filter op woordfrequentie
data_minfreq = amcat.features(conn, articleset_id=4073, unit_level='article', batchsize=100, min_freq=5) # gebruik alleen woorden die minstens [min_freq] voorkomen in corpus
data_minfreqperbatch = amcat.features(conn, articleset_id=4073, unit_level='article', batchsize=100, min_freq_perbatch=5) # features worden opgehaald per [batchsize] artikelen. min_freq_perbatch filtert op woordfrequentie per batch ipv over de hele corpus
